 ...
 * by the controllers that own the cascade path. Do NOT add a `CardAttachment`
 * or `Card` model observer that auto-deletes files — it will either
 ...
 */
trait CascadesCardFiles
{
    /**
     * Hard-delete a card AND its attachment files atomically.
     *
     * Sequence (in order):
     *   1. Snapshot every `card_attachments.path` BEFORE the row delete.
     *   2. `$card->delete()` — DB-level FK CASCADE removes
     *      `card_comments` AND `card_attachments` rows.
     ...
     */
    protected function deleteCardWithFileCascade(Card $card): void
    {
        DB::transaction(function () use ($card): void {
            ...
            // `AttachmentController::destroy` path, which deletes the file
